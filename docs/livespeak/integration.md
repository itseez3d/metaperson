...
> Integrate LiveSpeak into a web page via iframe — create the iframe, subscribe to events and add real-time voice and lip-sync to your MetaPerson 3D avatars.
...
| `camera_offset_x``camera_offset_y``camera_offset_z` | Camera position in metres, relative to the avatar's head. Overrides the preset value. |
| `camera_pitch``camera_yaw``camera_roll` | Camera rotation in degrees, relative to a head-on shot. Overrides the preset value. |
